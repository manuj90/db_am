...
function initProfilePage() {
        ...
        uploadBtn.addEventListener('click', function () {
                ...
                fetch(window.API_UPLOAD_URL || 'api/upload.php', { method: 'POST', body: formData })
                        ...
                        .catch(err => {
                                ...
                                uploadStatus.innerHTML = `<span class="text-red-400">${err.message || 'Error de red.'}</span>`;
                        })
                        .finally(() => {
                                uploadBtn.disabled = false;
                        });
...
function initEditProjectPage() {
        ...
        document.getElementById('btnCancelarEliminar')?.addEventListener('click', closeEliminarModal);
        document.getElementById('btnCancelarDuplicar')?.addEventListener('click', closeDuplicarModal);

        document.getElementById('modalEliminar')?.addEventListener('click', e => { if (e.target === e.currentTarget) closeEliminarModal(); });
        document.getElementById('modalDuplicar')?.addEventListener('click', e => { if (e.target === e.currentTarget) closeDuplicarModal(); });

        const inputArchivos = uploadForm.querySelector('input[name="nuevos_archivos[]"]');
        if (inputArchivos) {
        ...
        ['titulo', 'descripcion', 'cliente'].forEach(campo => {
                ...
                if (el) {
                        el.addEventListener('input', function () {
                                clearTimeout(autoSaveTimeout);
                                autoSaveTimeout = setTimeout(() => console.log('Auto-guardando borrador...'), 2000);
                        });
                }
        });
